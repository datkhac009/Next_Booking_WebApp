      ...
      <ul className="flex gap-2 overflow-x-auto pb-2 text-base lg:h-full lg:flex-col lg:pb-0 lg:text-lg">
        {navLinks.map((link) => (
          <li key={link.name} className="shrink-0">
            <Link
              className={`flex items-center gap-3 rounded-xl px-4 py-3 font-semibold text-primary-200 transition-colors hover:bg-primary-900 hover:text-primary-100 lg:gap-4 lg:px-5 ${pathName === link.href ? " bg-primary-900 " : ""}`}
              href={link.href}
            >
              {link.icon}
